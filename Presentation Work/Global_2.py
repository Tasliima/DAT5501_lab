...
highlight = ["United Kingdom", "China", "United States of America", "India", "Germany"]
...
for i, (country, color) in enumerate(zip(highlight, highlight_colors)):
    ...
    if not data.empty:
        ax1.bar(x + i*width, data["Working Hours"], width, label=f"{country} Hours", color=color, alpha=0.7)

ax1.set_xlabel("Year", fontsize=12)
ax1.set_ylabel("Average Annual Working Hours per Worker", fontsize=12)
ax1.set_xticks(x + width*2)  # center the grouped bars
...
for country, color in zip(highlight, highlight_colors):
    data = merged[merged["location_name"] == country].sort_values("year")
    if not data.empty:
        ax2.plot(x + width*2, data["Neurological Deaths (thousands)"],
